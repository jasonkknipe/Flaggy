...
### Compete attempt log (cross-device)

Compete attempts are kept locally as an offline cache and synchronised to a shared Supabase log when it is configured. There is no login or device partitioning: every installation sees the same attempt history.

1. Create a Supabase project and run [the migration](supabase/migrations/20260720000000_compete_attempts.sql) in its SQL editor.
2. In the GitHub repository, add Actions secrets named `VITE_SUPABASE_URL` (your project URL) and `VITE_SUPABASE_ANON_KEY` (the project's publishable/anon key).
3. Deploy normally. The workflow injects those public client settings at build time.

Because the log is intentionally shared, anyone using the deployed app can see its Compete attempts. Do not use this setup for private or personal data.
